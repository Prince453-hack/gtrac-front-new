...
const PadlockAnatomy = () => {
  return (
    <div
      className="min-h-screen w-full relative p-6 sm:p-12 lg:p-20 overflow-hidden flex flex-col justify-between"
      style={{
    ...
    >
      ...
      <motion.div
        ...
        className="relative z-20 max-w-xl mb-8 lg:mb-0"
      >
        <h2
          className={`${goldman.className} text-3xl sm:text-4xl md:text-5xl font-medium text-zinc-900 tracking-tight`}
        >
          Lock Anatomy
        </h2>
        <p
          className={`${poppins.className} text-xs sm:text-sm text-zinc-500 mt-3 leading-relaxed max-w-lg`}
        >
      ...
      <div className="relative w-full max-w-7xl mx-auto flex-1 min-h-137.5 lg:min-h-162.5 flex items-center justify-center my-6">
        <svg
        ...
        >
          <Image
            ...
            priority
            draggable={false}
            className="w-65 sm:w-75 md:w-85 h-auto object-contain drop-shadow-2xl"
          />
        </motion.div>

        {/* Callout 1: Hardened Shackle (Top-Left) */}
        <motion.div
          initial={{ opacity: 0, x: -30 }}
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6, delay: 0.2 }}
          className="static lg:absolute lg:top-[16%] lg:left-[8%] xl:left-[12%] z-30 max-w-xs mt-6 lg:mt-0"
        >
        ...
        {/* Callout 2: Charging Pin (Bottom-Left Card) */}
        <motion.div
          initial={{ opacity: 0, x: -30 }}
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6, delay: 0.3 }}
          className="static lg:absolute lg:bottom-[15%] lg:left-[4%] z-30 w-full max-w-xs mt-6 lg:mt-0"
        >
          <div
            className="backdrop-blur-md rounded-2xl py-10 px-5"
            style={{
              background:
        ...
        {/* Callout 3: Unlock Button (Middle-Right Card) */}
        <motion.div
          initial={{ opacity: 0, x: 30 }}
          whileInView={{ opacity: 1, x: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6, delay: 0.4 }}
          className="static lg:absolute lg:top-[42%] lg:right-[4%] z-30 w-full max-w-xs mt-6 lg:mt-0"
        >
          <div
            className="rounded-2xl py-10 px-5"
            style={{
              background:
